...
def init_ColdFilter(memory_kb):
    k = 3
    bits = [4, 16]  # counter bits for each layer
    thresholds = [15, 1000]

    memory_bits = memory_kb * 1024 * 8
    memory_bits_layer_1 = memory_bits // 2
    memory_bits_layer_2 = memory_bits - memory_bits_layer_1
    # Calculate counter numbers for each layer based on the allocated memory
    num_counters_layer_1 = memory_bits_layer_1 // bits[0]
    num_counters_layer_2 = memory_bits_layer_2 // bits[1]
